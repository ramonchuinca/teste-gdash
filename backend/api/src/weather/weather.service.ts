...
export class WeatherService {
  ...
  async create(data: Partial<Weather>): Promise<Weather> {
    try {
      ...
      this.logger.error('Erro ao criar registro', error);
  ...
  async findAll(limit = 100, skip = 0) {
    try {
      ...
      this.logger.error('Erro ao buscar registros', error);
      throw new Error('Failed to fetch weather records');
    }
  }

  /** Exporta dados de clima para CSV */
  async exportCSV() {
    try {
  ...
  /** Busca clima atual de uma cidade via API externa */
  async getCurrent(city: string, latitude: number, longitude: number) {
    try {
      ...
      return {
        ...
        humidity: data.humidity ?? null, // Open-Meteo nem sempre fornece
