...
let WeatherController = class WeatherController {
    ...
    async getCurrent(city, latitude, longitude, lat, lon) {
        const finalLat = Number(latitude ?? lat);
        const finalLon = Number(longitude ?? lon);
        if (!city) {
            throw new common_1.BadRequestException('O campo "city" é obrigatório.');
        }
        if (isNaN(finalLat) || isNaN(finalLon)) {
            throw new common_1.BadRequestException('Latitude e longitude precisam ser números válidos.');
        }
        return await this.weatherService.getCurrentWeather(city, finalLat, finalLon);
    }
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], WeatherController.prototype, "getAll", null);
